...
import sys
import os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))  # 添加父目錄到系統路徑
import random
...
class Map:
    def __init__(self, width, height, seed=None):
        ...
        self.width = width
        self.height = height
        self.tiles = [TILE_PATH for _ in range(self.width * self.height)]  # 初始化所有格子為路徑
    ...
    def add_central_room(self):
        ...
        start_y = (self.height - room_h) // 2  # 房間左上角 y 坐標
        ...
        for j, row in enumerate(room):
            for i, cell in enumerate(row):
                self.set_tile(start_x + i, start_y + j, cell)  # 設置房間格子
    ...
    def _check_if_surrounding_all_wall_type(self, x, y, wall_type):
        ...
        原理：
        - 檢查 (x, y) 格子周圍的八個格子（3x3 區域，排除中心點），是否全部為路徑（TILE_PATH）。
        - 用於確保牆壁生成點不會破壞迷宮結構。
    ...
    def place_power_pellets(self):
        ...
        if self.seed is not None:
            random.seed(self.seed + 1000)  # 使用獨立種子
    ...
    def generate_maze(self):
        ...
        原理：
        - 按以下步驟生成迷宮：
          1. extend_walls()：在左半部分隨機生成牆壁。
          2. add_central_room()：添加中央 5x5 房間。
