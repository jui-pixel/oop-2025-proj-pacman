...
def train(resume=False, model_path="pacman_dqn_final.pth", memory_path="replay_buffer.pkl", episodes=1000):
    ...
    state_dim = (env.maze.h, env.maze.w, 6)  # 狀態維度：迷宮高度、寬度、6 通道
    action_dim = len(env.action_space)  # 動作數量：4（上、下、左、右）
    ...
    # 載入先前模型（如果 resume=True 且檔案存在）
    if resume and os.path.exists(model_path):
        agent.load(model_path, memory_path)
        agent.epsilon = 0.3  # 提高探索率以避免局部最優
